...
void Init_Post_Filter(PofStatus *handle)
{
  handle->res2  = handle->res2_buf + PIT_MAX;
  handle->scal_res2  = handle->scal_res2_buf + PIT_MAX;

  handle->mem_pre = 0;
  handle->past_gain=4096;

  Set_zero(handle->mem_syn_pst, M);
  Set_zero(handle->res2_buf, PIT_MAX+L_SUBFR);
  Set_zero(handle->scal_res2_buf, PIT_MAX+L_SUBFR);
 ...
 Word32  L_tmp;
    ...
    *-----------------------------------------------------*/
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
    {
      ...
      Residu(Ap3, &syn[i_subfr], handle->res2, L_SUBFR);

      /* scaling of "res2[]" to avoid energy overflow */

      for (j=0; j<L_SUBFR; j++)
      {
        handle->scal_res2[j] = shr(handle->res2[j], 2);
      }

      /* pitch postfiltering */
      if (sub(Vad, 1) == 0)
        pit_pst_filt(handle->res2, handle->scal_res2, t0_min, t0_max, L_SUBFR, res2_pst);
      else
        for (j=0; j<L_SUBFR; j++)
          res2_pst[j] = handle->res2[j];
      ...
      preemphasis(&(handle->mem_pre), res2_pst, temp2, L_SUBFR);

      /* filtering through  1/A(z/GAMMA1_PST) */

      Syn_filt(Ap4, res2_pst, &syn_pst[i_subfr], L_SUBFR, handle->mem_syn_pst, 1);

      /* scale output to input */

      agc(&(handle->past_gain), &syn[i_subfr], &syn_pst[i_subfr], L_SUBFR);

      /* update res2[] buffer;  shift by L_SUBFR */

      Copy(&(handle->res2[L_SUBFR-PIT_MAX]), &(handle->res2[-PIT_MAX]), PIT_MAX);
      Copy(&(handle->scal_res2[L_SUBFR-PIT_MAX]), &(handle->scal_res2[-PIT_MAX]), PIT_MAX);
