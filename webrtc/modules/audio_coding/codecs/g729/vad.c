         ...
         Word16 *marker)
{
 /* scalar */
  ...
  if((frm_count & 0x0007) == 0){
    ...
    for (i=1; i<16; i++){
      if (sub(handle->Min_buffer[i], handle->Prev_Min) < 0)
  ...
  if(sub(frm_count, 129) >= 0){
    ...
    if (sub(ENERGY, handle->Min) < 0)
      handle->Min = ENERGY;
    if (sub(ENERGY, handle->Next_Min) < 0)
  ...
  if (sub(frm_count, INIT_FRAME) >= 0){
    ...
    if(handle->flag == 1){
      if((pprev_marker == VOICE) && 
         (prev_marker == VOICE) && 
         (*marker == NOISE) && 
         (sub(abs_s(sub(handle->prev_energy,ENERGY)), 614) <= 0)){
        handle->count_ext++;
        *marker = VOICE;
        handle->v_flag = 1;
        if(sub(handle->count_ext, 4) <= 0)
          handle->flag=1;
        else{
          handle->count_ext=0;
          handle->flag=0;
    ...
    if((*marker == VOICE) && (sub(handle->count_sil, 10) > 0) && 
       (sub(sub(ENERGY,handle->prev_energy), 614) <= 0)){
      *marker = NOISE;
      handle->count_sil=0;
    }
    ...
    if ((sub(sub(ENERGY, 614), handle->MeanSE) < 0) && (sub(frm_count, 128) > 0)
        && (!handle->v_flag) && (sub(rc, 19661) < 0))
      *marker = NOISE;

    if ((sub(sub(ENERGY,614),handle->MeanSE) < 0) && (sub(rc, 24576) < 0)
        && (sub(SD, 83) < 0)){ 
      handle->count_update++;
      if (sub(handle->count_update, INIT_COUNT) < 0){
        COEF = 24576;
        C_COEF = 8192;
        COEFZC = 26214;
        C_COEFZC = 6554;
        ...
        if (sub(handle->count_update, INIT_COUNT+10) < 0){
          COEF = 31130;
          C_COEF = 1638;
          COEFZC = 30147;
          C_COEFZC = 2621;
          ...
          if (sub(handle->count_update, INIT_COUNT+20) < 0){
            COEF = 31785;
            C_COEF = 983;
            COEFZC = 30802;
            C_COEFZC = 1966;
            ...
            if (sub(handle->count_update, INIT_COUNT+30) < 0){
              COEF = 32440;
              C_COEF = 328;
              COEFZC = 31457;
              C_COEFZC = 1311;
              ...
              if (sub(handle->count_update, INIT_COUNT+40) < 0){
                COEF = 32604;
                C_COEF = 164;
                COEFZC = 32440;
                C_COEFZC = 328;
    ...
    if((sub(frm_count, 128) > 0) && (((sub(handle->MeanSE,handle->Min) < 0) &&
                        (sub(SD, 83) < 0)) || (sub(handle->MeanSE,handle->Min) > 2048))){
