...
**
*/
void Init_Dec_cng(SidStatus *handle)
{
  
  Copy(freq_prev_reset, handle->lspSid, M);
  handle->sid_gain = tab_Sidgain[0];
...
)
{
  ...
  if(parm[0] != 0) {
    ...
    handle->sid_gain = tab_Sidgain[(int)parm[4]];           
    
    /* Inverse quantization of the LSP */
    sid_lsfq_decode(&parm[1], handle->lspSid, freq_prev);
  ...
  /*****************/
    ...
    if(dif == 0) {
      ...
      handle->sid_gain = tab_Sidgain[(int)ind];
    }
  ...
  Calc_exc_rand(handle->L_exc_err, handle->cur_gain, exc, seed, FLAG_DEC);

  /* Interpolate the Lsp vectors */
  Int_qlpc(lsp_old, handle->lspSid, A_t);
  Copy(handle->lspSid, lsp_old, M);
