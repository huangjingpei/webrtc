 ...
 *--------------------------------------*/
  ...
  for(i= 2; i<=M; i++)
  {
    ...
    if (abs_s(Kh) > 32750)
    {
      ...
      rc[0] = handle->old_rc[0];        /* only two rc coefficients are needed */
      rc[1] = handle->old_rc[1];
      return;
    }
  ...
  *Err = shr(alp_h, alp_exp);
  ...
  {
    t0   = L_Comp(Ah[i], Al[i]);
    handle->old_A[i] = A[i] = round(L_shl(t0, 1));
  }
  handle->old_rc[0] = rc[0];
  handle->old_rc[1] = rc[1];
