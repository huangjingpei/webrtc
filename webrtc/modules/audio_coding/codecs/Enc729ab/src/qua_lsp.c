...
)
{
  ...
  Lsp_qua_cs(handle, lsf, lsf_q, ana );
...
)
{
  ...
  Relspwed( flsp_in, wegt, lspq_out, lspcb1, lspcb2, fg,
    handle->freq_prev, fg_sum, fg_sum_inv, code);
}
...
void Get_freq_prev(EncStatus *handle, Word16 x[MA_NP][M])
{
  Word16 i;
...
void Update_freq_prev(EncStatus *handle, Word16 x[MA_NP][M])
{
  Word16 i;
